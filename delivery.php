...
<title>DELIVERY Traffic lights</title>
...
$sql = "SELECT top 1 * from qryTrafficLightDelivery";
...
echo '<table><thead>';
 ...
 echo '<Tr>';
    ...
    //echo '<strong>'.$key.'</strong> - '.$val . ',';
	    if ($key != 'Banner' && $key != 'Unit name' && $key != 'Loaded' && $key != 'ComboGroupID' && $key != 'impexplu' && $key != 'CollFlag' && $key != 'DelFlag'&& $key != 'From'){
		 echo '<th>'.$key.'</th>';	 
	  }
		if ($key == 'Unit name'){
 ...
 echo '</Tr>';
   ...
   echo '</thead>';
 $sql = "SELECT  * from qryTrafficLightDElivery";
$stmt = $dbh->prepare($sql);
$stmt->execute();
$colcnt = $stmt->columncount();
    ...
    foreach($row as $key=>$val)
    {
		switch ($key)
		{
			...
			*/
			...
			//get time
			$nextTime = $row['Del time'];
			

			$h = substr($nextTime,8,2);
			$min = substr($nextTime,11,2);
			//get date  have changed format of date to yyyy/mm/dd
			$next = $row['Del date'];
			$d = substr($next,-2);
			$m = substr($next,5,2);
			$y = substr($next,0,4);
			...
			//Loaded is ticked i.e. at the delivery point, all okay
				echo '<TD>'.$val .'Hrs' .'</TD>';
			} //EO if loaded 
			break;
